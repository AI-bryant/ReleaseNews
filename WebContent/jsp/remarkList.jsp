                ...
                <div class="widget-box">
                    ...
                    <div class="widget-content nopadding">
                        <table class="table table-bordered data-table">
                            ...
                            <tbody>
	                            <c:forEach var="remark" items="${requestScope.remarkList}">
	                           		<tr class="gradeX">
	                           		 	<c:forEach var="user" items="${requestScope.userList}">
			                                <c:choose>
										   		<c:when test="${remark.userId == user.userId}">
													 <td>${user.userName }</td>
										   		</c:when>
											</c:choose>
		                                ...
		                                <c:forEach var="news" items="${requestScope.newsList}">
			                                <c:choose>
										   		<c:when test="${news.newsId == remark.newsId}">
													 <td>${news.newsTitle }</td>
										   		</c:when>
											</c:choose>
		                                </c:forEach>
			                            <td>${remark.remarkContext }</td>
			                            <td class="center">${remark.createDate }</td>
		                            	<td class="center"><a href="MDeleteServlet?remarkId=${remark.remarkId }">删除</a></td>
	                           		</tr>
								</c:forEach>
                            </tbody>
                        </table>
