...
public class MLogServlet extends BasicServlet {
	private static final long serialVersionUID = 1L;
	private static final String MANAAGER_INDEX_PAGE = "MVIndexCountServlet";
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(userTemp != null){
			...
			if(user != null && "admin".equals(user.getUserStatus())){
				this.getSession(request).setAttribute("userManager", user);
				this.getSession(request).setAttribute("userManagerId", user.getUserId());
				response.sendRedirect(MANAAGER_INDEX_PAGE);
			}else{
